...
def build_input_data_from_word2vec(sentence, word2vec_vocab, word2vec_vec):
    ...
    for word in sentence:
        try:
            ...
            word2vec_index = word2vec_vocab['<un_known>']
            word_vector = word2vec_vec[word2vec_index]
            #word_vector = np.random.uniform(low=-0.00001, high=0.00001, size=len(word2vec_vec[0]))
...
def load_data(dataset_path, word2vec_model_path, n_class=2, max_seq_len_cutoff=1350):
    ...
    word2vec_vocab = {}
    word2vec_vec = []
    vector_file = open(word2vec_model_path, 'r')
    vector_content = vector_file.readlines()
    for index, word in enumerate(vector_content):
        if(index == 0):
            continue
        word = word.strip()
        word = word.split()
        ...
        word2vec_vec.append(float_vec)

        word2vec_vocab[line_word] = index-1



    word_vector = np.random.uniform(low=-0.25, high=0.25, size=(1,len(word2vec_vec[0])))
    float_vec = [v for v in word_vector]
    word2vec_vec.append(float_vec)
    tmpindex = len(word2vec_vec)-1
    word2vec_vocab['<un_known>'] = tmpindex
