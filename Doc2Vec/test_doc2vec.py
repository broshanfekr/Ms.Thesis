...
def test_Classifier(classifier, model, extra_model = None):
    ...
    if(extra_model == None):
        for sent in test_sentences:
            mydocwords = sent[0]
            myvec = model.infer_vector(mydocwords, alpha=0.05, min_alpha=0.0001, steps=25)
            test_sentences_vec.append(myvec)
    else:
        for sent in test_sentences:
            mydocwords = sent[0]
            first_vec = np.asarray(model.infer_vector(mydocwords, alpha=0.05, min_alpha= 0.0001, steps=25))
            sec_vec = np.asarray(extra_model.infer_vector(mydocwords, alpha=0.05, min_alpha= 0.0001, steps=25))
            test_sentences_vec.append(Concat_Paragraph_Vector(v1=first_vec, v2= sec_vec))

    #computing logistic regression score witch is in classifire[0]
    logistic_score = classifier[0].score(np.asarray(test_sentences_vec), np.asarray(test_labels))
    #computing NN Score witch is in classifire[1]
    nn_score = classifier[1].score(np.asarray(test_sentences_vec), np.asarray(test_labels))
    return logistic_score, nn_score
...
def main(argv):
    ...
    PV_DM_Model = Load_Model("./myIMDB_model_PV_DM.d2v")
    logging.info("PV_DM_Model Loaded successfully")
    Training_Algorithm = 0 # to build a pv_dbow model
    Sample_Frequency = 1e-4
    PV_DBOW_Model = Load_Model("./myIMDB_model_PV_DBOW.d2v")

    logging.info("PV_DBOW_Model Loaded successfully")
    ...
    score = test_Classifier([classifier, nn], PV_DM_Model , PV_DBOW_Model)

    destfile.write("Logistic Regression score is: " + str(score[0]))
    destfile.write("\n")
    print("Logistic Regression Score is: ", score[0])

    destfile.write("NN score is: " + str(score[1]))
    destfile.write("\n")
    print("NN Score is: ", score[1])
