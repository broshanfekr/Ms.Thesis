...
doc_list = alldocs[:]#train_plus_extra[:]  # for reshuffling per pass
...
simple_models = [
    ...
    Doc2Vec(dm=1, dm_concat=1, size=150, window=5,sample=1e-2, negative=25, hs=0, min_count=2, workers=cores),
    # PV-DBOW
...
def logistic_predictor_from_data(train_targets, train_regressors):
    ...
    #predictor = classifier.score(np.asarray(train_regressors), np.asarray(train_targets))
    ...
    for name, train_model in models_by_name.items():
        ...
        if ((epoch + 1) % 5) == 0 or epoch == 0:
            ...
            with elapsed_timer() as eval_elapsed:
                infer_err, err_count, test_count, predictor = error_rate_for_model(train_model, train_docs, test_docs,infer_subsample=1,
                                                                                   infer=True)
